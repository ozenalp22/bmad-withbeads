...
Always use the project-local `bd` binary:

```bash
# Preferred (project-local, guaranteed available)
_bmad/bin/bd <command>

# Fallback (system PATH - less reliable)
bd <command>
```
...
if ! _bmad/bin/bd version > /dev/null 2>&1; then
  echo "ERROR: Beads CLI not available. Run BMAD installer."
  exit 1
fi

# Verify beads is initialized
if [ ! -d ".beads" ]; then
  echo "ERROR: Beads not initialized. Run: _bmad/bin/bd init --quiet"
  exit 1
fi
```
